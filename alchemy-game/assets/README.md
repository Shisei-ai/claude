...
| `homunculi/<種類ID>.png` | ホムンクルスの肖像（ホムンクルスタブ）。種類ID: `h_worker` `h_saver` `h_twin` `h_sage` | 24×24 または 32×32 |
...
- **アイテムID**（装備を含む。装備は `staff1`〜`staff7` のように「系統＋段階」）: `herb`（霊草）、`wood`（古木の枝）、`stone`（石材）、`mushroom`（月光茸）、`sand`（赤砂）、`sunstone`（陽光石）、`iron_ore`（鉄鉱石）、`silver_ore`（銀鉱石）、`crystal`（魔晶の欠片）、`ice`（永久氷）、`frost_flower`（霜の花）、`soul_ash`（魂の残滓）、`nether_stone`（冥石）、`slime_gel`（粘液）、`wolf_fang`（獣の牙）、`bone`（古骨）、`miasma_core`（瘴気の核）、`nether_heart`（番人の心核）、`water`（清水）、`essence`（草の精髄）、`charcoal`（木炭）、`glass`（硝子瓶）、`mana`（魔素）、`iron_powder`（鉄粉）、`silver_powder`（銀粉）、`sun_dust`（陽光の粉）、`bone_powder`（骨粉）、`iron_ingot`（鉄塊）、`silver_ingot`（銀塊）、`pure_water`（聖水）、`pure_core`（浄化核）、`enchanted_iron`（魔鉄）、`gold`（錬金の金）、`alkahest`（万能溶媒）、`salt`（賢者の塩）、`mercury`（賢者の水銀）、`sulfur`（賢者の硫黄）、`quintessence`（第五元素）、`red_tincture`（赤きティンクトゥラ）、`potion`（回復薬）、`mana_potion`（魔力薬）、`lantern`（霧払いの灯）、`frost_amulet`（耐寒の護符）、`nether_key`（冥府の鍵）、`philosopher_stone`（賢者の石）、`stone_shard`（賢者の石の欠片）、`abyss_shard`（深淵の欠片）、`void_essence`（虚無の雫）、`primal_ember`（原初の火種）、`forest_core`（古王の樹核）、`desert_core`（大蠍の毒晶）、`mine_core`（巨像の魔核）、`snow_core`（狼王の氷牙）、`boss_soul`（主の魂晶）、`abyss_heart`（深淵の核）、`staff1`（樫の魔杖）、`staff2`（銀晶の杖）、`staff3`（魔鉄の杖）、`staff4`（金冥の杖）、`staff5`（賢者の杖）、`staff6`（星詠みの杖）、`staff7`（原初の杖）、`robe1`（旅の外套）、`robe2`（聖布の法衣）、`robe3`（冥布の法衣）、`robe4`（虚無の衣）、`robe5`（第五元素の衣）、`robe6`（深淵の外套）、`robe7`（原初の衣）、`cat_fire1`（紅蓮の触媒 I）、`cat_fire2`（紅蓮の触媒 II）、`cat_fire3`（紅蓮の触媒 III）、`cat_ice1`（凍晶の触媒 I）、`cat_ice2`（凍晶の触媒 II）、`cat_ice3`（凍晶の触媒 III）、`cat_thunder1`（雷霆の触媒 I）、`cat_thunder2`（雷霆の触媒 II）、`cat_thunder3`（雷霆の触媒 III）、`cat_wind1`（疾風の触媒 I）、`cat_wind2`（疾風の触媒 II）、`cat_wind3`（疾風の触媒 III）、`cat_light1`（煌光の触媒 I）、`cat_light2`（煌光の触媒 II）、`cat_light3`（煌光の触媒 III）、`cat_cycle1`（循環の触媒 I）、`cat_cycle2`（循環の触媒 II）、`cat_cycle3`（循環の触媒 III）、`cat_swift1`（迅速の触媒 I）、`cat_swift2`（迅速の触媒 II）、`cat_swift3`（迅速の触媒 III）、`cat_life1`（生命の触媒 I）、`cat_life2`（生命の触媒 II）、`cat_life3`（生命の触媒 III）
- **設備ID**: `pot`（抽出の壺）、`furnace`（精錬の竈）、`mortar`（破砕の臼）、`cauldron`（融合の釜）、`purifier`（浄化の匣）、`enchanter`（魔化の匣）、`transmuter`（変成の祭壇）、`forge`（錬装の炉）
