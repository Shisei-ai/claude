...
// 習得方法: 各グレードに対応する魔術書を読むこと(BOOKS の unlock.spells に 'fire:2' のように記載)
//   M.Lv が上がると読める書物が増える → 書物を読む → 記載された術を習得、という流れ
const SPELLS = {
  ...
  light: { ch: '光', color: '#fff7c2', type: 'beam', length: 640, desc: '神の理を否定する光線。貫通する。',
    grades: [
      ...
      { name: '煌輝', dmg: 380, mp: 56, cd: 3.4, width: 52 },
    ],
  },
};
...
// ---- 魔術書(奥義): 魔法の上位グレードを記した書物 ---------------
// grade: [魔法ID, グレード, 必要M.Lv, 書名, 本文]
const GRADE_BOOKS = [
  ['bolt', 2, 5,  '重ね撃ちの理', ['魔弾は一つである必要はない。', '二つの弾を同時に編む──【重魔弾】。狙いの甘さは数で補えばよい。']],
  ['bolt', 3, 11, '穿孔魔弾論', ['弾に「螺旋」を与えよ。', '回転する魔力は肉を、骨を、盾をも穿ち抜ける。【穿魔弾】は並ぶ敵をまとめて貫く。']],
  ['bolt', 4, 18, '星を撃つ者', ['夜空の星は、太古の魔術師が撃ち上げた魔弾の名残だという。', '【星魔弾】──三条の光は、ついに星へ届いた者の術である。']],
  ['fire', 2, 7,  '業火の書', ['火球に怒りを込めれば、炎は業火となる。', 'ただし怒りに呑まれるな。【業火球】は術者の心をも焼く。']],
  ['fire', 3, 13, '煉獄の門扉', ['死者の罪を焼き清める炎、煉獄。', '【煉獄球】の爆炎は、魔物の瘴気ごと灼き尽くす。']],
  ['fire', 4, 20, '劫火黙示録', ['世界の終わりに降るという劫火。', 'これを人の手で放つ【劫火】──三つの炎が地を舐め、何も残さない。']],
  ['heal', 2, 8,  '快癒の祈祷書', ['癒しの祈りを重ねよ。一度より二度、二度より三度。', '【快癒】は深い傷をも塞ぐ。']],
  ['heal', 3, 14, '聖なる癒し手', ['聖水と浄化核の研究から生まれた、癒しの到達点のひとつ。', '【聖癒】──穢れを祓いながら肉体を繕う。']],
  ['heal', 4, 21, '再生の秘蹟', ['失われたものは戻らない。だが、在るべき姿へは戻せる。', '【再生の祈り】は、錬金術の理を癒しに転じた秘蹟である。']],
  ['ice', 2, 11, '氷牙の書', ['氷の槍に牙を刻め。', '【氷牙槍】は刺さった敵の熱を奪い、より長く足を止める。']],
  ['ice', 3, 16, '凍獄記', ['永久雪原の最奥には、あらゆるものが凍てついた「獄」がある。', 'その冷気を写し取った【凍獄槍】は、二本同時に放たれる。']],
  ['ice', 4, 22, '絶対零度の証明', ['全ての動きが止まる温度。時さえも。', '【絶対零度】──止める力の、その極み。']],
  ['thunder', 2, 12, '迅雷の書', ['雷は速い。だが、もっと速くできる。', '【迅雷鎖】はより多くの敵へと跳ね回る。']],
  ['thunder', 3, 17, '轟雷教典', ['雷を崇める古い教団の経典。', '【轟雷鎖】の轟きは、群れ全体を一度に打ち据える。']],
  ['thunder', 4, 23, '天雷の書', ['天の怒りと呼ばれたものを、今度はこちらが振るう番だ。', '【天雷】──十の敵を貫く雷の奔流。']],
  ['nova', 2, 13, '旋風の書', ['風陣をさらに速く、さらに広く巡らせよ。', '【旋風陣】は包囲を許さない。']],
  ['nova', 3, 17, '嵐刃の型', ['風の刃に「型」を与えることで、斬撃は嵐となる。', '【嵐刃陣】の間合いに踏み入る者はいない。']],
  ['nova', 4, 24, '天嵐の書', ['何者にも縛られぬ風。神の理にさえも。', '【天嵐】──己を中心に天の嵐を喚ぶ。']],
  ['light', 2, 16, '否定の光', ['反証から否定へ。神の理に「否」を突きつける光。', '【否定の光】は、より太く、より深く届く。']],
  ['light', 3, 21, '棄却の光', ['否定の先にあるのは棄却。その理は、もはや存在しない。', '【棄却の光】は神の遣いすら退ける。']],
  ['light', 4, 26, '煌輝', ['全ての理を棄て去った後に残る、ただ一つの輝き。', '【煌輝】──それは、誰かを取り戻そうとした者だけが辿り着く光である。']],
];
for (const [spell, g, lv, name, text] of GRADE_BOOKS) {
  BOOKS.push({ id: `g_${spell}${g}`, kind: 'magic', series: spell, lv, sp: 1, name, unlock: { spells: [`${spell}:${g}`] }, text });
}
