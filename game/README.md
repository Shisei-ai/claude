...
- **設備（盤面に配置）** … 資源入力（鉱区の素材を取り込む）／製錬炉（素材2→合金）／部品工場（合金→ボディ/ヘッド/ウェポン）
  ／組立機（3部品→機体）／コンベア／発電機／研究所／通信中継塔／モジュール工房／防衛砲台。建設コストは**鉄**。
- **コンベア** … ドラッグで敷設し、流れる向きを引く。機械の出力側（矢印）から下流へアイテムが運ばれる。
- **電力** … 発電機が供給、各設備が消費。供給を超えると全設備の稼働速度が低下する。
- **採取モジュール** … 各鉱区に投入した数で素材の産出量が変わる。モジュール工房で製造し、研究／暫時防衛で**保有上限**が拡張。
- **組立機の設計** … 組立機を選んで「ボディ＋ウェポン＋ヘッドCPU」を設定。指揮容量の範囲で機体を連続生産する。

ラインの引き回し、製錬・部品・組立の台数バランス、電力――これらが戦闘中の**増援が湧き続ける速度**を決める。
