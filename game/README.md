...
Canvas による自前の描画エンジンで、戦場が常に動く。

- **多層背景** … 空のグラデーション、敵側の赤いグロー、ドリフトする星、遠近の稜線、
  奥行きを出す地面グリッド、基地シルエット、ビネット。
- **ユニット作画** … 兵科ごとに形状（重装＝六角／突撃＝三角／歩兵＝円）。
  歩行で脚が動き、砲身が敵方向を向く。**空中ユニットはローターが回転**し係留線とスラスター炎を描く。
- **敵作画** … 脈動・うごめく触手・赤いコア。ボスは回転スパイクと発光コア。
...
└─ js/
   ...
   └─ ui.js          描画（Canvas）・工業UI・モーダル・メインループ
```
